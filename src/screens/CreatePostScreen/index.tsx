...
const CreatePostScreen = ({navigation, route}: any) => {
  ...
  const bookId = useRef<string>(uuidv4());
  const uuid = useRef<string>(auth().currentUser!.uid);
  const reference = storage().ref(`BookPictures/${bookId.current}.png`);
  ...
  const [linkToBook, setLinkToBook] = useState<string>('');
  ...
  return (
    <SafeAreaView style={{flex: 1, backgroundColor: COLORS.white}}>
      <View style={styles.header}>
        ...
        />
        <Text style={styles.titleHeader}>Tạo bài viết</Text>
      </View>
      <TouchableOpacity
        style={[
          {
        ...
        <Image
          ...
          resizeMode="contain"
          source={pathImage ? {uri: pathImage} : images.picture}
        />
        {!pathImage && (
          <Text
            style={{
